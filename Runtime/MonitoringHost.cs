...
namespace UnityEssentials
{
    public class MonitoringHost : GlobalSingleton<MonitoringHost>
    {
        ...
        private static readonly Dictionary<ulong, MonitorTarget> s_targetMap = new();
            ...
            for (var i = Targets.Count - 1; i >= 0; i--)
            {
                if (!Targets[i].IsAlive())
                {
                    if (Targets[i].TargetInstance is UnityEngine.Object uo)
                        s_targetMap.Remove(EntityId.ToULong(uo.GetEntityId()));
                    Targets.RemoveAt(i);
                }
            }
        ...
        public static void RegisterTarget(MonoBehaviour mb)
        {
            ...
            var id = EntityId.ToULong(mb.GetEntityId());
